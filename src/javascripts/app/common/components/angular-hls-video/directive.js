...
module.exports = [
  ...
  function($sce) {
    return {
      ...
      template:
        '<div><video class="video-to-play" preload="metadata"></video></div>',
      link: function(scope, element, attr) {
        scope.$watch("streamingSrc", function(newValue, oldValue) {
          var outerDiv = element.children()[0];
          var videoElement = document.getElementsByClassName(
            "video-to-play"
          )[0];
          ...
          videoElement.setAttribute(
            "height",
            isProperHeight ? scope.height : "270"
          );
          videoElement.setAttribute(
            "width",
            isProperWidth ? scope.width : "480"
          );
          var canPlayHls = videoElement.canPlayType(
